...
describe("custom segment registry", () => {
  ...
  test("resolves Pi runtime packages like extension modules", async () => {
    const dir = createTempSegmentsDir();
    writeFile(dir, "pi-runtime.ts", `
      ...
      export default function ({ registerSegment }) {
        ...
        registerSegment({
          id: 'pi_runtime',
          render() {
            return { content: getAgentDir().length > 0 && schema.type === 'object' ? 'pi-ok' : 'missing', visible: true };
          },
        });
      }
    `);
    ...
    expect(result.errors).toEqual([]);
    expect(renderSegment("pi_runtime", createSegmentContext())).toEqual({
      content: "pi-ok",
      visible: true,
    });
  });
