...
function writePowerlinePresetSetting(preset: StatusLinePreset): boolean {
  const settings = readSettingsForWrite("powerline-footer", "preset");
  if (!settings) {
    return false;
  }

  updatePowerlineSettings(settings, (current) => ({
    ...current,
    preset,
  }));

  return persistSettings(settings, "powerline-footer", "preset");
}
...
function resolveShortcutConfig(settings: NormalizedPowerlineSettings): PowerlineShortcuts {
...
function applyPowerlineConfig(settings: NormalizedPowerlineSettings): void {
  config = {
    settings,
