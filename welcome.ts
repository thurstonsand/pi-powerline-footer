...
export class WelcomeComponent implements Component {
  ...
  render(termWidth: number): string[] {
    ...
    const hChar = "─";
    const bottomLine =
      dim(hChar.repeat(Math.max(0, leftPad))) + countdownStyled + dim(hChar.repeat(Math.max(0, rightPad)));
...
export function discoverLoadedCounts(): LoadedCounts {
  ...
  const settingsPaths = [join(homeDir, ".pi", "agent", "settings.json"), join(cwd, ".pi", "settings.json")];
  ...
  const skillDirs = [join(homeDir, ".pi", "agent", "skills"), join(cwd, ".pi", "skills"), join(cwd, "skills")];
