...
3. Bump the fork package version without letting npm create its own commit or tag.
   ...
   For this fork, prefer upstream-version suffixes like `0.5.4-1`, `0.5.4-2`, then reset to `0.5.5-1` when rebasing onto upstream `0.5.5`.
...
- npm requires an explicit `--tag` for prerelease-style versions such as `0.5.4-1`; use `--tag latest` when this fork release should be the default install target.
